...
export async function GET() {
  ...
  const r = await pool.query(
    `SELECT a.nombre_activo, a.tipo, a.estado, c.nombre_categoria, ar.nombre_area, a.codigo_qr, a.observaciones_iniciales,
  ...
  for (const row of r.rows) {
    csv += `"${row.nombre_activo}","${row.tipo}","${row.estado}","${row.nombre_area || ''}","${row.nombre_categoria || ''}","${row.codigo_qr || ''}","${(row.observaciones_iniciales || '').replace(/"/g,'""')}","${row.componentes}"\n`;
  }
